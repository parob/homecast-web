...
describe('validateAutomationName', () => {
  it('rejects trailing punctuation with a suggested valid name (mirrors cloud suite)', () => {
    expect(() => validateAutomationName('Aircon Off Every 2 Hours (9am-9pm)'))
      .toThrow(/end with a letter or number.*Try "Aircon Off Every 2 Hours 9am-9pm"/s);
    expect(() => validateAutomationName('Lights (evening)')).toThrow(/Try "Lights evening"/);
    expect(() => validateAutomationName('Good night!!')).toThrow(/Try "Good night"/);
  });

  it('accepts valid names unchanged (inner punctuation is fine)', () => {
    expect(validateAutomationName('Aircon Off Every 2 Hours 9am to 9pm')).toBe('Aircon Off Every 2 Hours 9am to 9pm');
    expect(validateAutomationName('Heating (winter) schedule')).toBe('Heating (winter) schedule');
    expect(validateAutomationName('  Wake up 7  ')).toBe('Wake up 7');
  });

  it('rejects names with no letters or numbers (no suggestion possible)', () => {
    expect(() => validateAutomationName('!!!')).toThrow(/end with a letter or number/);
    expect(() => validateAutomationName('   ')).toThrow(/end with a letter or number/);
  });

  it('blocks bad names before anything reaches the bridge (no half-created debris)', async () => {
    mockHome();
    await expect(handleCreateAutomation({
      home: HOME_SLUG,
      name: 'Test (evening)',
      ...
      actions: [{ accessory: LIGHT_SLUG, on: true }],
    })).rejects.toThrow(/Try "Test evening"/);
    expect(HomeKit.createAutomation).not.toHaveBeenCalled();
  });
});
