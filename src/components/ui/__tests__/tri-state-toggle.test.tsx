...
describe('TriStateToggle — where the thumb sits', () => {
  ...
  // The whole point is legible at a glance, so the three positions are pinned:
  // the track is w-10 and the thumb w-4, which puts the midpoint at exactly
  // translate-x-3 between translate-x-1 and translate-x-5.
  const thumb = (root: HTMLElement) =>
    Array.from(root.querySelectorAll('span')).find(s => s.className.includes('translate-x-'))!;
  const fill = (root: HTMLElement) =>
    Array.from(root.querySelectorAll('span')).find(s => s.style.width !== '')!;

  it.each([
    ['off', 'translate-x-1', '0%'],
    ['mixed', 'translate-x-3', '50%'],
    ['on', 'translate-x-5', '100%'],
  ] as const)('parks %s at %s with the track %s filled', (state, offset, width) => {
    const { container } = render(<TriStateToggle state={state} onCheckedChange={vi.fn()} />);
    const root = container.firstElementChild as HTMLElement;
    expect(thumb(root).className).toContain(offset);
    expect(fill(root).style.width).toBe(width);
  });
